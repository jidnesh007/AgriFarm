  ...
  return (
    <div className="flex h-screen bg-[#F0FDF4] relative overflow-hidden font-sans">
      ...
      >
        ...
        {/* Logo & Toggle Section */}
        <div
          className={`p-5 relative z-10 flex items-center ${
            isSidebarCollapsed ? "flex-col gap-4" : "justify-between"
          }`}
        >
        ...
        <div
          className={`p-4 mt-auto border-t border-emerald-800/50 relative z-10 bg-emerald-950/30 rounded-br-3xl`}
        >
          <div
          ...
          <button
            ...
            } bg-emerald-800 hover:bg-red-500/80 hover:text-white text-emerald-100 rounded-lg transition-all duration-300 text-sm font-medium`}
          ...
          {loading ? (
            ...
            <>
              ...
              {currentView === "settings" && (
                <PlaceholderView title={t("nav.settings")} />
              )}
            </>
          )}
        </main>
...
const NavItem = ({ icon, label, active, onClick, badge, isCollapsed }) => (
  <button
    ...
    } gap-3 px-4 py-4 rounded-xl transition-all duration-300 group relative ${
      active
        ? "bg-emerald-50 text-emerald-900 shadow-lg"
  ...
  >
    ...
    {isCollapsed && active && (
      <div className="absolute left-0 w-1 h-6 bg-white rounded-r-full shadow-[0_0_8px_white]"></div>
    )}
  </button>
);
